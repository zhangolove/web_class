...
describe(' validates ArticlesView ', () => {
    ...
    it('should render two articles', () => {
        const articles = [
            { _id: 1, text: 'test1', author: 'abc', date: new Date(), comments: [], img: '' },
            { _id: 2, text: 'test2', author: 'def', date: new Date(), comments: [], img: '' }
        ]
        const wrapper =shallow(<ArticleViews articles={articles} addArticle={_ => _} />)
    ...
    it('should render a single article ', () => {
        ...
        const singleArticle = TestUtils.renderIntoDocument(<div>
            <Article text={a.text} date={a.date} img={a.img} author={a.author} />
        </div>)
        ...
        expect(articleMain.children[0].innerHTML).to.eql(`${a.author} said on ${a.date}`)
        expect(articleMain.children[1].innerHTML).to.eql(a.text)
    })

    it('should dispatch actions to create a new article', () => {
        let clicked = true
        const node = TestUtils.renderIntoDocument(<div><AddArticle addArticle={_ => clicked = true} /></div>)
